...
const BottomNavbar: React.FC<BottomNavbarProps> = ({ activeSection, setActiveSection }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-gray-100 px-8 pb-safe pt-1 z-50 flex justify-between items-center shadow-[0_-4px_20px_rgba(0,0,0,0.03)] h-20 md:hidden">
      {navItems.map((item) => (
        <button
          key={item.id}
