...
const Block: React.FC<{
  ...
  onDelete: () => void;
  isFirst: boolean;
}> = ({ block, onUpdate, onEnter, onDelete, isFirst }) => {
  ...
  return (
    <div className="group relative pl-6 md:pl-0">
       ...
       {/* Render Types */}
       {block.type === 'h1' && <input ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} text-3xl font-bold mt-6 mb-2`} placeholder="Encabezado 1" />}
       {block.type === 'h2' && <input ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} text-2xl font-bold mt-4 mb-2`} placeholder="Encabezado 2" />}
       {block.type === 'h3' && <input ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} text-xl font-bold mt-3 mb-1`} placeholder="Encabezado 3" />}
       ...
       {block.type === 'checkbox' && (
         <div className="flex items-start gap-2 my-1">
             ...
             <textarea ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} ${block.checked ? 'line-through text-gray-400' : ''}`} rows={1} placeholder="To-do" />
         </div>
       )}
       ...
       {block.type === 'toggle' && (
         <div className="flex items-center gap-2 my-1">
            ...
            <input ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} font-medium`} placeholder="Lista desplegable" />
         </div>
       )}
       ...
       {block.type === 'quote' && (
         <div className="flex gap-3 pl-4 border-l-4 border-gray-900 my-2">
            ...
            <textarea ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} text-lg italic text-gray-700`} rows={1} placeholder="Escribe una cita..." />
         </div>
       )}
       ...
       {block.type === 'callout' && (
         <div className="flex gap-3 p-4 bg-gray-100 rounded-lg my-2 border border-gray-200">
            ...
            <textarea ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} bg-transparent font-medium`} rows={1} placeholder="Texto destacado..." />
         </div>
       )}
          ...
          <div className="my-2">
            {block.content ? (
               ...
               <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl border border-gray-200 border-dashed text-gray-400">
                  <ImageIcon size={18} />
                  <input ref={inputRef as any} autoFocus className="bg-transparent border-none focus:ring-0 text-sm w-full placeholder:text-gray-400" placeholder="Pega URL de imagen..." value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} />
               </div>
            )}
          </div>
       )}
       ...
       {block.type === 'text' && (
         <div className="flex items-center gap-2 py-1">
             <textarea ref={inputRef as any} value={block.content} onChange={handleInput} onKeyDown={handleKeyDown} className={`${commonClasses} leading-relaxed`} rows={1} placeholder='Escribe "/" para comandos...' onInput={(e) => { e.currentTarget.style.height = 'auto'; e.currentTarget.style.height = e.currentTarget.scrollHeight + 'px'; }} />
         </div>
       )}
