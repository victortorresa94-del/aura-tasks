...
const TopicToDocModal: React.FC<TopicToDocModalProps> = ({ isOpen, onClose, userId, onNoteCreated }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const res = await aiService.createDocFromTopic(userId, topic, {
                template,
                length,
                prompt
            });
            ...
            <div className="relative w-full max-w-2xl bg-[#1A1A1A] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh] animate-scale-in">
                ...
                <div className="flex items-center justify-between p-6 border-b border-white/5 bg-gradient-to-r from-blue-500/10 to-transparent">
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-blue-500/20 text-blue-400 rounded-xl shadow-lg shadow-blue-500/10 ring-1 ring-blue-500/20">
                            <Type size={24} />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-white tracking-tight">Documento desde Tema</h3>
                            <p className="text-xs text-gray-400 font-medium">Investigación y redacción IA</p>
                        </div>
                    </div>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    {step === 'input' && (
                        <div className="space-y-4">
                            <div className="group">
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2 tracking-wider">Tema Principal</label>
                                <input
                                    value={topic}
                                    onChange={e => setTopic(e.target.value)}
                                    placeholder="Ej: Nutrición para maratonistas, Historia del Jazz..."
                                    className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl p-4 text-sm text-white focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all font-medium placeholder:text-gray-700"
                                    autoFocus
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Ej: Incluye ejemplos prácticos y evita tecnicismos..."
                    ...
                    {step === 'processing' && (
                        <div className="flex flex-col items-center justify-center py-16 text-center animate-fade-in">
                            <div className="relative mb-8">
                                ...
                                <div className="relative z-10 p-4 bg-[#0A0A0A] rounded-2xl border border-blue-500/20 shadow-2xl shadow-blue-500/10">
                                    <Wand2 size={48} className="text-blue-500 animate-spin-slow" />
                                </div>
                            </div>
                            <h4 className="text-2xl font-bold text-white mb-3 tracking-tight">Investigando...</h4>
                            <p className="text-sm text-gray-500 max-w-sm mx-auto leading-relaxed">
                                Nuestra IA está estructurando la información y redactando el contenido sobre tu tema.
                            </p>
                            <div className="mt-8">
                                <button onClick={() => setStep('input')} className="text-xs text-blue-400 hover:text-blue-300 font-bold tracking-wide uppercase px-4 py-2 rounded-lg hover:bg-blue-500/10 transition-colors">
                                    Cancelar Operación
                                </button>
                            </div>
                        </div>
...
export default TopicToDocModal;
