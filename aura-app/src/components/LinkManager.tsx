    ...
    <div className="space-y-3">
      ...
      {isSearching && (
        <div className="relative animate-fade-in-up">
           <div className="flex items-center gap-2 bg-gray-50 border border-indigo-200 rounded-lg px-2 py-1.5 focus-within:ring-2 focus-within:ring-indigo-100">
             <Search size={14} className="text-gray-400" />
             <input 
               autoFocus
               className="bg-transparent border-none text-sm w-full focus:ring-0 p-0 placeholder:text-gray-400"
               placeholder="Buscar tarea, nota, persona..."
               value={searchTerm}
               onChange={e => setSearchTerm(e.target.value)}
             />
             <button onClick={() => setIsSearching(false)}><X size={14} className="text-gray-400" /></button>
           </div>
             ...
             <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 shadow-xl rounded-lg max-h-48 overflow-y-auto z-50">
               {allEntities.length > 0 ? allEntities.map(item => (
                 <button 
                   ...
                   className="w-full text-left px-3 py-2 hover:bg-indigo-50 flex items-center gap-2 border-b border-gray-50 last:border-0"
                 >
                   {getIcon(item.type)}
                   ...
                   <span className="text-xs text-indigo-600 opacity-0 group-hover:opacity-100">Vincular</span>
                 </button>
               )) : (
      ...
      <div className="flex flex-wrap gap-2">
        {currentLinks.map(link => (
          <div key={link.id} className="group flex items-center gap-2 pl-2 pr-1 py-1 bg-white border border-gray-200 rounded-lg shadow-sm hover:border-indigo-300 transition-colors">
             {getIcon(link.type)}
             <span className="text-xs font-medium text-gray-700 max-w-[150px] truncate">{link.title}</span>
             <button 
               onClick={() => removeLink(link.id)}
               className="p-1 hover:bg-red-50 hover:text-red-500 rounded text-gray-300 transition-colors"
             >
               <X size={12} />
             </button>
          </div>
        ...
        {currentLinks.length === 0 && !isSearching && (
          <p className="text-xs text-gray-400 italic">Sin conexiones activas.</p>
        )}
      </div>
    </div>
  );
